...
def upgrade():
    ...
    harvest_source_column_names = [
        column["name"] for column in inspector.get_columns("harvest_source")
    ]
    if "time" not in harvest_source_column_names:
        op.add_column("harvest_source", sa.Column("time", sa.UnicodeText))
